...
def database():
    ...
    with conn:
        cursor = conn.cursor()
    cursor.execute(
        'CREATE TABLE IF NOT EXISTS PIC (nameofPIC TEXT,nameofstudent TEXT,course TEXT,SAPID INTEGER,RollNo TEXT, internship TEXT, interncompany TEXT, optforplacement TEXT, placementrecieved TEXT, placedcompany TEXT)')
    cursor.execute(
        'CREATE TABLE IF NOT EXISTS CC (nameofCC TEXT,nameofstudent TEXT,certificationDone TEXT, Achievements TEXT)')
    cursor.execute(
        'CREATE TABLE IF NOT EXISTS AC (nameofAC TEXT,nameofstudent TEXT,IV_visited TEXT,IVlocation TEXT,minor1 TEXT, mentor1 TEXT, minor2 TEXT, mentor2 TEXT, major1 TEXT, mentormajor1 TEXT, major2 TEXT, mentormajor2 TEXT)')
    cursor.execute(
        'INSERT INTO PIC (nameofPIC,nameofstudent,course,SAPID,RollNo,internship, interncompany, optforplacement, placementrecieved, placedcompany ) VALUES(?,?,?,?,?,?,?,?,?,?)',
        (nameofPIC, nameofstudent, course, SAPID, RollNo, internship, interncompany, optforplacement, placementrecieved,
         placedcompany))
    cursor.execute('INSERT INTO CC (nameofCC,nameofstudent,certificationDone, Achievements ) VALUES(?,?,?,?)',
                   (nameofCC, nameofstudent, certificationDone, achievements))
    cursor.execute(
        'INSERT INTO AC (nameofAC,nameofstudent,IV_visited,IVlocation,minor1, mentor1, minor2, mentor2, major1, mentormajor1, major2, mentormajor2) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',
        (
        nameofAC, nameofstudent, IV_visited, IVlocation, minor1, mentor1, minor2, mentor2, major1, mentormajor1, major2,
        mentormajor2))
    conn.commit()
